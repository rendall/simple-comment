...
- [ ] C05 `[backend]` If `strictNullChecks`-related failures surface while running `yarn test:backend`, apply minimal typing fixes in the failing `src/tests/backend/**/*.ts` files without relaxing test intent/assertions. Depends on: C03, C04.
