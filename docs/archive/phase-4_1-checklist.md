...
- [x] C01 `[governance]` Confirm Phase 4.1 remains scoped to `noImplicitAny` only (no `strictNullChecks`/`strict` toggle, no broad env/string-throw refactor) per `docs/archive/phase-04-type-safety-and-env-handling.md`.
...
- [x] C06 `[docs]` Record the Phase 4.2 strictness gate decision in `docs/archive/phase-04-type-safety-and-env-handling.md` with one of: proceed to `strictNullChecks` or pause for hotspot cleanup, including deferred `TODO(phase-04.2)` hotspots (if any). Depends on: C05.
