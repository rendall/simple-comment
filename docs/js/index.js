...
import { postComment, getDiscussion, getCurrentUser, deleteAuth, postAuth, getOneUser, getDefaultDiscussionId, createNewTopic, getOneTopic } from "./apiClient.js";
const clearStatus = () => {
...
const setStatus = (message, isError = false) => {
    if (typeof message !== "string") {
        if (typeof message.text === "function") {
            ...
            return setStatus(JSON.stringify(message), isError);
    }
    else {
...
const clearReply = () => {
    ...
    if (oldSubmitReplyButton)
        oldSubmitReplyButton.remove();
    const oldCancelReply = document.querySelector("#reply-cancel-button");
    if (oldCancelReply)
        oldCancelReply.remove();
};
...
const onReplyToComment = comment => e => {
    ...
    submitReplyButton.addEventListener("click", onSubmitReply(textarea, comment.id));
...
const onReceiveDiscussion = discussion => {
    ...
    const threadReplies = (listItem, parentId) => {
        ...
        if (comment)
            attachComment(comment, listItem);
...
const getSelf = (userObj) => getOneUser(userObj.user).then(setUserStatus).catch(setErrorStatus);
const onLoginClick = e => {
    const usernamevalue = document.querySelector("#userid")
        .value;
...
const updateLoginStatus = () => getCurrentUser()
    .then(getSelf)
    .catch(currUserError => {
...
const downloadDiscussion = discussionId => getOneTopic(discussionId).then(resp => {
...
const tryCreatingTopic = (discussionId, title) => createNewTopic(discussionId, title)
    .then(() => setStatus("Topic created!"))
    .then(() => downloadDiscussion(discussionId))
    .catch(err => {
    if (err.status === 401) {
        setErrorStatus("Simple Comment policy disallows anonymous discussion creation.");
    }
    else
        setErrorStatus(err);
});
const setup = (discussionId = getDefaultDiscussionId(), title = document.title) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!simpleCommentArea) {
        console.error("Simple Comment area not found. q.v. https://github.com/rendall/simple-comment ");
        return;
    }
