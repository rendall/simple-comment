...
import { AdminSafeUser, PublicSafeUser, Success, UserId, Error } from "../lib/simple-comment"
import { getNewUserInfo, getTargetId, getUpdatedUserInfo, getUserId } from "./modules/helpers"
dotenv.config()
...
export const handler = async (event: APIGatewayEvent, context: APIGatewayEventRequestContext) => {
  ...
  const handleMethod = (method):Promise<Success<(PublicSafeUser | AdminSafeUser) | (AdminSafeUser[] | PublicSafeUser[])> | Error> => {

    switch (method) {
      case "GET": {
        if (targetId) 
        return service.userGET(targetId, authUserId)
        else return service.userListGET(authUserId)
      }
      case "POST": return service.userPOST(getNewUserInfo(event.body), authUserId)
      case "PUT": return service.userPUT(targetId, getUpdatedUserInfo(event.body), authUserId)
      case "DELETE": return service.userDELETE(targetId, authUserId)
