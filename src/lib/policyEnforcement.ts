 ...
 * */
export const isUserAllowedTo = (userId:UserId, action:ActionType, p:Policy = policy):true | string => {

  switch (action) {
    case Action.postComment:
      if (!p.isGuestAccountAllowed && isGuestId(userId)) return "Guest accounts are forbidden to comment by policy isGuestAllowed: false"
      return true
