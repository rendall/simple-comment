...
export const hashPassword = (password:string) => crypto.createHmac('sha256', process.env.HASH_SECRET).update(password).digest('hex')

export const comparePassword = (plainTextPassword: string, hash: string) => hashPassword(plainTextPassword) === hash
